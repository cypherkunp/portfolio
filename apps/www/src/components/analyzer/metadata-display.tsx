...
export function MetadataDisplay({ metadata }: MetadataDisplayProps) {
  ...
  return (
    <motion.div
      ...
      className="border-border bg-card flex flex-wrap items-start gap-6 rounded-lg border px-5 py-4 md:gap-10"
    >
      ...
      {metadata.license && <MetadataField label="License">{metadata.license}</MetadataField>}

      {authorText && <MetadataField label="Author">{authorText}</MetadataField>}

      {metadata.description && (
        <MetadataField label="Description">
          <span className="text-muted-foreground max-w-xs">{metadata.description}</span>
        </MetadataField>
      )}
